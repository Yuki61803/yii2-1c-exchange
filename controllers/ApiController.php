...
class ApiController extends Controller
{
    ...
    public function beforeAction($action)
    {
        if ($this->module->debug) {
            $monitor = new Monitor();
            $monitor->url = Yii::$app->request->url;
            $monitor->route = $this->route;
    ...
    public function serializeResponse($result)
    {
        if (is_bool($result)) {
    ...
    public function afterAction($action, $result)
    {
        ...
        $result = $this->serializeResponse($result);
        if ($this->module->debug && $this->_monitorModel) {
            $this->_monitorModel->http_code = Yii::$app->response->statusCode;
            $this->_monitorModel->response = $result;
            $this->_monitorModel->save();
        }
        ...
        switch ($type) {
            case 'catalog':
                ...
                foreach (glob($dir) as $file) {
                    if (strpos($file, 'classifier')) {
                        // Не удаляем и не переименовываем файл классификатора, он может потребоваться по ходу выгрузки
                        // а инициализация может повторяться неоднократно
                        continue;
                    }
                    if ($this->module->debug) {
    ...
    public function parsingImport($file)
    {
        ...
        if ($commerce->classifier->xml) {
            ...
            $classifierId = (string)$commerce->catalog->ИдКлассификатора;
            $classifierFile = $this->module->getTmpDir() . DIRECTORY_SEPARATOR . "classifier-{$classifierId}.xml";
            if (!file_exists($classifierFile)) {
                Yii::error("Файл классификатора $classifierFile не найден", 'exchange1c');
                throw new Exception('File classifier.xml not found');
            }
    ...
    public function actionQuery($type)
    {
        ...
        if ($this->module->exchangeDocuments) {
            ...
            if ($this->module->debug) {
                ...
                file_put_contents($this->module->getTmpDir() . '/query-' . date('Y-m-d H:i:s') . '.xml', $xml);
            }
        }
